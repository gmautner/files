...
while read -r line; do
    ...
    line="${line//\"/}"
    # If the image reference is implicit (no registry specified), add the appropriate prefix.
    if [[ "$line" != *"/"* ]]; then
        # No slash means it's an official image from Docker Hub
        line="docker.io/library/${line}"
    else
        # If the part before the first slash does not look like a registry (i.e. contains no dot or colon and is not "localhost"),
        # then prefix it with docker.io
        registry="${line%%/*}"
